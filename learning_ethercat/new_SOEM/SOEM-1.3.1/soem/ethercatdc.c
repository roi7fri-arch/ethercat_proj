...
void print_sync0()
{
	uint16 slaveh;
    int32 tc;
    (void)ecx_FPRD((&ecx_context)->port, 0x1001, ECT_REG_DCCYCLE0, sizeof(tc), &tc, EC_TIMEOUTRET); /* read local time of slave */
    printf("SYNC0 cycle time is %d\n", tc);
}
...
void ecx_dcsync0(ecx_contextt *context, uint16 slave, boolean act, uint32 CyclTime, uint32 CyclShift)
{
   ...
   t1 = etohll(t1);
   printf("local time of slave 1 is %ld\n", t1);
   ...
   if (CyclTime > 0)
   {
	   ...
	   printf("first trigger time is %ld\n", t);
   }
   else
   {
...
tc = 0;
 (void)ecx_FPRD(context->port, slaveh, ECT_REG_DCCYCLE0, sizeof(tc), &tc, EC_TIMEOUTRET); /* read local time of slave */
    printf("SYNC0 cycle time before is %d\n", tc);
   tc = htoel(CyclTime);
   ...
   (void)ecx_FPRD(context->port, slaveh, ECT_REG_DCCYCLE0, sizeof(tc), &tc, EC_TIMEOUTRET); /* read local time of slave */
    printf("SYNC0 cycle time is %d\n", tc);
   (void)ecx_FPRD(context->port, slaveh, ECT_REG_DCCYCLE1, sizeof(tc), &tc, EC_TIMEOUTRET); /* read local time of slave */
    printf("SYNC1 cycle time is %d\n", tc);
}
