...
//	out_ELMO_EL->position_demand = torque_val;
	out_ELMO_EL->position_offset = pos;
	out_ELMO_EL->velocity_offset = vel;
	out_ELMO_EL->torque_offset = torque;
}
...
{
	float trqcm= 1.0;
	out_ELMO_platt *out_ELMO;
	/*Here we get pointer to iomap - slave I/O pointers - pointer for SM2 outputs*/
	out_ELMO = (out_ELMOt*)ec_slave[ELMO_EL].outputs;
	out_ELMO->control_word_axis1.enableOperation = 1;
	out_ELMO->control_word_axis1.switchOn = 1;
	out_ELMO->control_word_axis1.quickStop = 1;
	out_ELMO->control_word_axis1.enableVoltage= 1;
	//out_ELMO_EL->target_torque = torque_val;//calcTargetTorque(trqcm);
//	out_ELMO_EL->position_demand = torque_val;
	out_ELMO->position_offset_axis1 = pos_axis_1;
	out_ELMO->velocity_offset_axis1 = vel_axis_1;
	out_ELMO->torque_offset_axis1 = torque_axis_1;

	out_ELMO->control_word_axis2.enableOperation = 1;
	out_ELMO->control_word_axis2.switchOn = 1;
	out_ELMO->control_word_axis2.quickStop = 1;
	out_ELMO->control_word_axis2.enableVoltage= 1;
	//out_ELMO_EL->target_torque = torque_val;//calcTargetTorque(trqcm);
//	out_ELMO_EL->position_demand = torque_val;
	out_ELMO->position_offset_axis2 = pos_axis_2;
	out_ELMO->velocity_offset_axis2 = vel_axis_2;
	out_ELMO->torque_offset_axis2 = torque_axis_2;
}
...
//	out_ELMO_TR->position_demand = torque_val;
	out_ELMO_TR->position_offset = pos;
	out_ELMO_TR->velocity_offset = vel;
	out_ELMO_TR->torque_offset = torque;
}
